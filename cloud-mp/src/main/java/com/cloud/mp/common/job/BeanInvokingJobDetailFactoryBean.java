...
public class BeanInvokingJobDetailFactoryBean
  implements FactoryBean<JobDetail>, BeanNameAware, InitializingBean, ApplicationContextAware
{
    ...
    throws Exception
  {
    try
    {
      ...
      JobBuilder jobBuilder = JobBuilder.newJob(
        this.concurrent ? BeanInvokingJob.class : StatefulBeanInvokingJob.class)
      ...
      logger.debug("Registering JobListener names with JobDetail object " + this.beanName);
      if (this.jobListenerNames != null)
      {
        for (int i = 0; i < this.jobListenerNames.length; i++);
      }

      logger.info("创建任务[JobDetail]: " + this.jobDetail + "; targetBean: " + this.targetBean + "; targetMethod: " + this.targetMethod + "; arguments: " + this.arguments + ";");
    }
    finally
    {
      logger.debug("end");
    ...
    implements Job
  {
    ...
    {
      String targetBean = null; String targetMethod = null;
      try
      {
        logger.debug("start");
        ...
        Object bean = BeanInvokingJobDetailFactoryBean.applicationContext.getBean(targetBean);
  ...
  public static class StatefulBeanInvokingJob extends BeanInvokingJobDetailFactoryBean.BeanInvokingJob
